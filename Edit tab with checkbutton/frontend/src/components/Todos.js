...
const Todos = () => {
  ...
  const handleChecked = (id)=>{
    axios.put(`http://localhost:8080/update-status/${id}`)
    .then(result =>{
...
navigate(`/edit-todo/${id}`)
  ...
  return(
    <div>
      ...
      <ul>
        {(post.length === 0) ? (<h2 className='font-bold text-center text-blue-700 py-2 text-xl my-3'>No Tasks Pending!</h2>): ( post.map((todo) => (
          <li key={todo._id}>
